...
1. **`src/backend/internal/`**：Go 的 `internal` 包有特殊的可见性规则——只有 `backend/` 目录下的代码能导入它。这强制了分层边界，防止外层代码绕过 Service 层直接调 Repository。
...
```bash
$ cd src/frontend
$ npx openapi-typescript ../../docs/openapi.yaml -o src/shared/api/types.gen.ts
```
