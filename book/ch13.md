...
从本章开始，我们进入前端部分。前12章主要讲 Go 后端、Docker Compose、项目架构和方法学，**没有系统讲过 TypeScript、JSX 和 React**。为了避免后面的代码示例变成“天书”，我们先花一节时间把必需的前置知识快速过一遍。

这一节的目标是**够用即可**：不讲透每个知识点的全部细节，但让你能读懂本章代码、知道每行在做什么。后续章节会在真实场景中反复强化这些概念。

## 13.0 前置知识速查：TypeScript、JSX 与 React Hooks 速览

### 13.0.1 TypeScript 核心语法

我们项目的前端用 **TypeScript** 编写。它在 JavaScript 基础上增加了**静态类型检查**：你在写代码时声明变量、函数参数、返回值的类型，编译器会在运行前帮你发现类型错误。

**变量与函数类型**

```ts
// 文件: src/frontend/src/examples/types-basic.ts（教学示例）

let count: number = 0
let title: string = '研究报告'
...
`interface` 定义了一个对象应该长什么样。`status: 'pending' | 'running' | ...` 是**联合类型**，表示这个字段只能是这几个字符串之一。
...
const u2: User = { name: 'Bob', bio: '全栈开发者' }
```

**泛型 `<T>`**

泛型让你写一个“类型占位符”，具体类型在使用时再确定。
...
const n = firstItem<number>([1, 2, 3]) // T 是 number
const s = firstItem(['a', 'b']) // TypeScript 自动推断 T 是 string
```

在 React 组件里，泛型常用于可复用的容器组件或 Hook。

**类型断言 `as`**

当你比 TypeScript 更清楚某个值的类型时，可以用 `as` 告诉编译器：
...
> **注意**：`as` 是“你告诉编译器信任我”，它不会真的改变运行时的值。如果用错了，运行时仍然会报错。
...
`import type` 只导入类型信息，不导入运行时代码。这样可以让打包工具更准确地做 tree-shaking。

### 13.0.2 JavaScript/ES6+ 速查

React 代码大量依赖现代 JavaScript 语法。下面是本章会用到的核心特性。
...
箭头函数在事件处理、回调、自定义 Hook 中非常常见。

**解构赋值**

```ts
const report = { title: '调研报告', topic: 'AI', status: 'completed' }

// 从对象中取出字段
const { title, status } = report

// 函数参数里直接解构
function ReportCard({ title, status }: { title: string; status: string }) {
...
```ts
const [count, setCount] = [0, (v: number) => {}]
```

React 的 `useState` 返回的就是一个二元数组，所以用数组解构接收。
...
```ts
const name = '报告'
const message = `正在生成 ${name}...` // 用反引号和 ${} 插值
```
...
`await` 必须写在 `async` 函数内部。它让异步代码看起来像同步代码。

**可选链 `?.` 与空值合并 `??`**

```ts
const user = { profile: { name: 'Alice' } }
const nickname = user.profile?.nickname // undefined，不会报错
...
JSX 是 React 的标记语法，看起来很像 HTML，但有几个重要区别。
...
function Greeting({ name }: { name: string }) {
  return <p>你好，{name.toUpperCase()}</p>
}
```

`{}` 里只能放**表达式**，不能放语句（如 `if`、`for`）。条件渲染通常用三元运算符或逻辑与：

```tsx
<div>
  {isAdmin ? <AdminPanel /> : <Login />}
  {showHint && <p>提示信息</p>}
</div>
```
...
因为 `class` 是 JavaScript 保留字，JSX 里用 `className`：
...
组件只能返回一个根元素。如果不想多包一层 `div`，可以用 Fragment：
...
`e.target.value` 就是输入框当前的值。

### 13.0.4 React Hooks 预览

Hooks 是 React 16.8 引入的机制，让函数组件也能拥有状态、副作用、引用等能力。本章会提前用到几个 Hook，这里只做最小预览，**第14章会系统讲解**。
...
`useState(0)` 返回一对值：`count` 是当前状态，`setCount` 是更新函数。
...
`useEffect` 用于和外部系统同步（定时器、网络请求、事件监听）。第二个参数 `[]` 表示“只在组件挂载时执行一次”。
...
`useRef` 常用于获取 DOM 节点，或在多次渲染之间保存一个不会触发重新渲染的值。
...
`useCallback` 用于保持函数引用稳定，避免子组件不必要的重新渲染。

### 13.0.5 Vite 与开发环境

我们前端使用 **Vite** 作为构建工具。Vite 提供了一个开发服务器，支持热更新、TypeScript 编译、静态资源处理等功能。

**环境变量**

Vite 通过 `import.meta.env` 暴露环境变量，只有以 `VITE_` 开头的变量才会被注入到客户端代码：
...
在开发环境下，Vite 可以把 `/api/*` 请求代理到后端服务。所以 `fetch('/api/health')` 在开发时会被转发到 `http://localhost:8080/api/health`。

> **注意**：Vite 配置在 `src/frontend/vite.config.ts` 里。本章不会修改它，但你要知道 `/api` 路径能工作是因为有这个代理。
...
| 知识点 | 本章覆盖程度 | 后续深入章节 |
|--------|--------------|--------------|
| TypeScript 类型系统 | 基础语法够用 | 全书示例中不断强化 |
| JSX 表达式与条件渲染 | 基本规则 | 第16章表单、第17章路由 |
| `useState` / `useEffect` | 最小预览 | **第14章 Hooks 完全指南** |
| `useRef` / `useCallback` | 最小预览 | **第14章 Hooks 完全指南** |
| `useMemo` | 未涉及 | **第14章 Hooks 完全指南** |
| `useReducer` / 自定义 Hooks | 未涉及 | **第14章 Hooks 完全指南** |
| 组件组合 / children / Context | 基础概念 | **第15章 状态管理**、第16章表单 |
| Server Components / Client Components | 概念建立 | 第17章路由、第20章复杂交互 |
| Actions / useActionState / useOptimistic | 应用示例 | **第16章 表单处理与数据校验** |
| Tailwind CSS / Shadcn UI | 未涉及 | **第18章、第19章** |

如果你已经熟悉以上内容，可以直接跳到 13.1。如果有些概念还是模糊，没关系，本章每个代码示例都会配合文字解释，后续章节还会反复遇到它们。
