...
|------|------|
| `src/backend/` | Go 后端代码 |
| `src/backend/cmd/` | 可执行程序入口（main 包） |
| `src/backend/internal/` | 私有业务代码（handler、service、repository） |
| `src/backend/pkg/` | 可被外部引用的公共库 |
| `src/frontend/` | React 前端代码 |
| `docs/` | 设计文档、API 文档 |
| `src/scripts/` | 构建脚本、迁移脚本 |
...
**`volumes`**。 named volume 让数据在容器重启后不丢失。`postgres_data` 是 Docker 管理的卷，不用关心具体路径。`./src/scripts/init.sql` 是绑定挂载，容器启动时会执行这个 SQL 文件。
...
创建 `src/backend/internal/config/config.go`：
