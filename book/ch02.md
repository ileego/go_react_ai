...
| `Makefile` | 文件 | 根目录聚合 Makefile（命令转发到前后端子 Makefile） |
...
项目根目录的 `Makefile` 只做一件事：根据命令前缀，转发到对应的子 Makefile。
...
你也可以分别 cd 到 `src/backend/` 或 `src/frontend/` 直接执行 `make run` 或 `make dev`，不经过根目录的转发。
