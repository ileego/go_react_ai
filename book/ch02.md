...
| `src/backend/Makefile` | 文件 | 后端常用命令封装 |
| `src/backend/.env.example` | 文件 | 环境变量模板（需复制为 `.env` 后使用） |
| `Makefile` | 文件 | 根目录代理 Makefile（命令转发到 `src/backend/Makefile`） |
...
我们在 `src/backend/` 下创建主 Makefile，存放所有后端相关的命令定义：
...
同时在**项目根目录**创建一个代理 Makefile，方便在根目录执行命令：

```makefile
# 文件: Makefile（项目根目录）
# 代理 Makefile：命令转发到 src/backend/Makefile
.PHONY: help env up down restart logs ps backend frontend test migrate
...
> ⚠️ 上面的主 Makefile（`src/backend/Makefile`）是 Windows 语法（`if not exist`、`timeout`、`copy`）。macOS/Linux 用户需要改成对应的 shell 语法。后面可以写个跨平台的 `src/scripts/dev.sh` 脚本来处理这个差异。
...
# 2. 生成环境文件并编辑（根目录的代理 Makefile 会自动转发到 src/backend/）
$ make env
