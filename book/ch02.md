...
| `docker-compose.yml` | 文件 | 开发环境基础设施定义 |
| `docker-compose.test.yml` | 文件 | 测试环境基础设施定义 |
| `Makefile` | 文件 | 常用命令封装 |
| `.env.example` | 文件 | 环境变量模板（需复制为 `.env` 后使用） |
...
```yaml
# 文件: docker-compose.test.yml（项目根目录）
services:
  postgres-test:
    image: ankane/pgvector:v0.5.1
    environment:
