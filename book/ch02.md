...
```bash
$ cd src/backend
$ go mod init github.com/ileego/go_react_ai
```
...
可用的对话模型：
- `GPT-5.5`（2026-04）：OpenAI 当前旗舰模型，推理、编程、多模态综合能力最强
- `GPT-5.4`（2026-04）：性能接近 GPT-5.5，价格更优，适合大多数生产场景
- `GPT-5.4-mini` / `GPT-5.4-nano`：轻量版，延迟低、成本低，适合高频调用
- `o3` / `o4-mini`：o 系列推理模型，STEM 和复杂逻辑任务表现突出
- `GPT-4.1` / `GPT-4.1-mini` / `GPT-4.1-nano`：前一代主力，上下文窗口达 100万 tokens，仍广泛使用
...
Claude 的特点是上下文长度长（Claude 4 Sonnet 支持 256K tokens），擅长长文档分析和代码生成。缺点是 API 在国内访问不稳定，价格也偏高。

模型选择：
- `claude-opus-4.8`（2026-05）：Anthropic 当前最强模型，推理和编程能力顶尖
- `claude-sonnet-4.6`（2026-05）：综合能力均衡，速度快，推荐默认使用
- `claude-3-7-sonnet`（2025-02）：前一代主力，性价比优秀，上下文 200K tokens
...
- `deepseek-v4`（2026-04）：当前旗舰，综合能力和推理性能大幅提升
- `deepseek-v3.1`（2025-08）：通用对话，价格便宜，适合大多数场景
- `deepseek-reasoner`：专攻数学/代码等推理任务
...
Kimi 的核心优势是**超长上下文窗口**：
- `kimi-k2-6`（2026-04）：当前旗舰，支持 256K 上下文，综合能力最强
- `kimi-k2-5`（2026-01）：支持 128K 上下文，性价比高
- `moonshot-v1-128k`：前一代模型，仍兼容可用

128K 意味着什么？大约可以一次性输入 20 万汉字，整本书直接丢进去。K2.6 的 256K 上下文更是翻倍。这在深度研究场景非常有价值——你可以让 Kimi 通读一本 300 页的 PDF，然后问它任意问题，而不用分块检索。
...
| AI 模型 | GPT-5.4-mini（省钱） | GPT-5.4 | GPT-5.4 / claude-sonnet-4.6 |
| API 重试次数 | 1（快速失败） | 3 | 5 |
...
> 这个 Makefile 使用标准 shell 语法，在 macOS、Linux 和 Windows（通过 Git Bash / WSL）下都能正常工作。`$(EXE)` 变量会根据操作系统自动添加或省略 `.exe` 后缀。
...
# 2. 生成后端环境文件并编辑
$ make backend-env
# 编辑 src/backend/.env 填入你的 AI API Keys
...
你也可以分别 cd 到 `src/backend/` 或 `src/frontend/` 直接执行 `make run` 或 `make dev`，不经过 `src/Makefile` 的转发。
