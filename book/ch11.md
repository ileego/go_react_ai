...
- 监控 GIN 索引大小，避免膨胀。
- 中文场景可考虑 `pg_bigm` 或 `zhparser` 优化分词（见 11.2.8）。

### 11.2.8 中文分词优化

#### 当前方案的局限

我们在迁移里使用了 `to_tsvector('simple', ...)`。`simple` 字典对英文只是做小写化，对中文则是**按单字切分**：

```sql
SELECT to_tsvector('simple', '人工智能在医疗领域的应用');
-- 结果类似：'人':1 '工':2 '智':3 '能':4 '在':5 '医':6 '疗':7 '领':8 '域':9 '的':10 '应':11 '用':12
```

这意味着用户搜索 `"人工智能"` 时，数据库实际在找同时包含 `"人"`、`"工"`、`"智"`、`"能"` 的文档。它会命中"人工智能"，也可能因为某篇报告里同时出现了"工人""智慧"而被误召回。简单说：**召回率高、精确率低**。

如果平台以中文内容为主，建议在生产环境引入中文分词插件。

#### 方案一：pg_bigm（推荐，运维简单）

`pg_bigm` 是 PostgreSQL 的 n-gram 索引扩展，对中日韩文本友好。它把文本按连续 N 个字符切片（默认 2-gram）：
...
- 无需维护词典；
- 对中英文混合、专有名词、新词效果好；
- 安装简单，很多 PostgreSQL 发行版可直接 `apt install postgresql-16-pg-bigm`。

缺点：

- 索引体积比 `tsvector` 大；
- 不支持排名（rank），适合"是否包含"的过滤查询，不适合相关性排序。
...
-- 用 bigm 索引替代 GIN tsvector 索引
CREATE INDEX idx_reports_bigm ON reports USING GIN (title gin_bigm_ops);
...
#### 方案二：zhparser（效果更好，运维稍重）

`zhparser` 基于 SCWS 中文分词库，能按词切分：

```text
"人工智能在医疗领域的应用" → "人工智能" "医疗" "领域" "应用"
```
...
- 分词精度高；
- 支持自定义词典，可加入领域术语；
- 与 PostgreSQL 全文搜索体系完全兼容，支持 `ts_rank` 排名。

缺点：

- 需要额外安装 SCWS 和 zhparser；
- Docker 镜像需要自定义构建或挂载扩展；
- 需要维护词典以应对新词。
...
```sql
-- 安装扩展后创建解析配置
CREATE TEXT SEARCH CONFIGURATION zh (COPY = pg_catalog.simple);
ALTER TEXT SEARCH CONFIGURATION zh ALTER MAPPING FOR word WITH zhparser;

-- 修改触发器使用中文解析
NEW.search_vector :=
    setweight(to_tsvector('zh', COALESCE(NEW.title, '')), 'A') ||
    setweight(to_tsvector('zh', COALESCE(NEW.topic, '')), 'B') ||
    setweight(to_tsvector('zh', COALESCE(NEW.content, '')), 'C');
```

#### 本书的选择

考虑到教学重点和开发环境开箱即用，本书当前实现继续使用 `simple` 字典。它足以演示 PostgreSQL 全文搜索的完整流程，且不需要修改 Docker 镜像。

如果你的平台中文内容占主导，建议按以下路径演进：

1. 初期：`simple` 字典 + GIN 索引（本书现状）；
2. 中期：引入 `pg_bigm` 做快速中文包含查询；
3. 后期：自定义 Docker 镜像安装 `zhparser`，获得精确分词和相关性排名。
