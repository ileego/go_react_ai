...
- `tailwind.config` / `tailwind.css`（即 `tailwind` 对象下的 `config` 和 `css` 字段）：`config` 指向 Tailwind 配置文件，`css` 指向项目的 CSS 入口；Shadcn CLI 在初始化或更新主题变量时会读取并修改这个 CSS 文件，因此它必须与项目里实际引入 Tailwind 指令的文件（本章即 `src/index.css`）保持一致；
...
这种写法在 2–4 个变体时还能接受，但问题在于：

1. **变体之间没有类型约束**：拼写错误要到运行时才暴露；
2. **难以组合尺寸**：如果要加 `sm` / `lg`，需要再写一套 `stylesSm`、`stylesLg`；
3. **外部覆盖困难**：调用方想传 `className` 覆盖某个属性时，需要手动处理类名冲突。

`cva` 解决了这三个问题：

```tsx
buttonVariants({ variant: 'default', size: 'sm', className: 'w-full' })
// 输出：基础类名 + default 变体 + sm 尺寸 + w-full，冲突部分由 tailwind-merge 处理
```

> **提示**：`cva` 不是 Tailwind 专属工具。它只负责"按条件拼接类名字符串"，因此也可以用于 CSS Modules 或其他原子类方案。
...
直接在所有页面里使用 `import { Button } from '@/components/ui/button'` 也可以，但项目中已经有很多地方调用自建的 `Button`：
...
如果一次性把所有调用点改成 Shadcn 的 `variant="default"` / `variant="destructive"`，改动量大且容易出错。更好的做法是保留 `src/shared/components/Button.tsx` 作为薄包装：

```tsx
export function Button({ children, variant = 'primary', size, ...rest }) {
  const shadcnVariant = variantMap[variant] ?? variant
  return <ShadcnButton variant={shadcnVariant} size={size} {...rest}>{children}</ShadcnButton>
}
```
...
- 旧调用点无需修改，编译即可通过；
- 新代码可以直接使用 Shadcn Button 的全部能力；
- 项目有一个统一的"按钮入口"，后续如果换用其他组件库，只需改这一个文件。

这种"薄包装 + 底层组件"的分层，是组件工程化的典型模式。

### 19.3.3 表单三件套：Input / Label / Textarea / Select

原生输入框最大的问题是每个项目都要重新写一遍焦点环、错误态、标签对齐。第16章我们用原生控件 + RHF 写表单时，`CreateReportFormRHF.tsx` 里每个字段都长这样：
...
1. **样式不一致**：不同开发者写的 `className` 可能不同，项目里很快出现 `border-gray-300`、`border-slate-600`、`border` 等多种边框写法；
2. **焦点环缺失或冲突**：原生 `input` 的默认轮廓被 `outline-none` 去掉后，如果没有 `focus-visible:ring`，键盘用户就看不到焦点在哪；
3. **错误态需要手动维护**：错误时不仅要显示文字，还应该把边框变红、增加 `aria-invalid`；
4. **暗色模式需要写大量 `dark:`**：每个颜色类都要考虑明暗两套。
...
注意这里用到的全是语义化变量：`border-input`、`bg-background`、`text-muted-foreground`、`ring-ring`。它们都来自 `tailwind.config.cjs` 的 `colors` 扩展和 `src/index.css` 的 CSS 变量定义。
...
第16章我们用 `register` 和 `Controller` 处理表单。Shadcn 的 `Input`、`Textarea` 都通过 `forwardRef` 暴露 ref，因此可以直接用 `register`：

```tsx
<Input {...register('title')} />
```

`Select` 没有原生 `value/onChange` 的简洁绑定，需要配合 `Controller` 或 Shadcn Form 的 `FormField`（19.3.4 会详细讲）。

> **注意**：`forwardRef` 是 Shadcn 组件能被 RHF、`react-select` 等库正常使用的关键。如果自定义组件没有正确转发 ref，RHF 的 `register` 会失效。
...
#### FormField 与 Controller 的对应关系

第16章我们学过 RHF 的 `Controller`：
...
| 能力 | RHF `Controller` | Shadcn `FormField` |
| ---- | ---------------- | ------------------ |
| 受控绑定 | 手动 `field.value` / `field.onChange` | 相同 |
| 标签关联 | 手动写 `htmlFor` + `id` | `FormLabel` + `FormControl` 自动处理 |
| 错误提示 | 手动判断 `formState.errors` | `FormMessage` 自动显示 |
| 错误样式 | 手动写红色边框/文字 | `aria-invalid` + 预设样式自动生效 |
| 字段说明 | 手动写辅助文本 | `FormDescription` 统一位置 |

> **注意**：Shadcn Form 并没有替代 RHF，它只是把"每个字段重复的样板代码"抽成组件。你仍然使用 `useForm`、`handleSubmit`、`resolver`、`defaultValues` 等 RHF API。
...
在第16、17章的登录页和创建报告页中，错误提示通常是这样写的：
...
- 颜色硬编码 `text-red-600`，暗色模式下可能对比度不足；
- 没有图标、没有标题，重要信息不突出；
- 没有语义化角色，屏幕阅读器不一定能自动朗读。

`Alert` 组件通过 `variant="destructive"` 自动切换背景、边框、文字色，并带有 `role="alert"`，错误信息更醒目、更可访问。

#### Badge 的状态映射

在 `ReportCard.tsx` 中，我们用 `Badge` 替换原来手写的一堆 `statusClasses`：
...
改造后代码量更少，且暗色模式、焦点状态、可访问性都由 `Badge` 组件统一处理。

### 19.3.6 Dialog 与 Sheet 的区别

Shadcn 提供了两种常用的覆盖层组件：

| 组件 | 交互模式 | 适用场景 | 与路由的关系 |
| ---- | -------- | -------- | ------------ |
| `Dialog` | 模态弹窗，阻断当前流程 | 确认删除、表单弹窗、重要提示 | 通常是当前页面的状态 |
| `Sheet` | 从侧面滑出的抽屉 | 移动端导航、详情面板、筛选器 | 也可以是当前页面的状态 |

本章用 `Sheet` 做移动端导航抽屉。为什么选择 Sheet 而不是 Dialog？

1. **导航不需要完全阻断用户**：Drawer 式抽屉更符合移动端的导航心智模型；
2. **空间利用**：Sheet 可以占满屏幕高度，容纳更多链接；
3. **与顶部导航的对应关系**：抽屉里的链接和顶部导航复用同一组数据，只是展示方式不同。

```tsx
// 文件: src/frontend/src/routes/root.tsx（教学示例片段）
      ...
      <div className="mt-6 flex flex-col gap-2">
        {navItems.map((item) => (
          <SheetClose key={item.to} asChild>
            <Link to={item.to} className="rounded-md px-3 py-2 text-sm font-medium transition-colors hover:bg-accent">
              {item.label}
            </Link>
          </SheetClose>
        ))}
...
这里复用了第17章学到的 `Link` 组件和第18章学到的响应式隐藏 `md:hidden`。`SheetClose` 包裹 `Link` 后，点击链接会自动关闭抽屉，提升移动端体验。
...
Dialog 会强制用户做出选择，适合高风险操作。本章虽然没有直接引入 `Dialog`，但理解了 Sheet 与 Dialog 的差异，后续章节需要确认弹窗时可以自然接入。
