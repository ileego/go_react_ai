...
func (r *EmbeddingRepository) Create(ctx context.Context, reportID int64, chunks []string, vectors [][]float32) error {
	...
	stmt, err := tx.PrepareContext(ctx, `
		INSERT INTO report_embeddings (report_id, chunk_index, content, embedding, created_at)
		VALUES ($1, $2, $3, $4, $5)
	...
	for i, content := range chunks {
		if _, err := stmt.ExecContext(ctx, reportID, i, content, pgvector.NewVector(vectors[i]), now); err != nil {
...
func (r *EmbeddingRepository) SearchSimilar(ctx context.Context, reportID int64, queryVector []float32, limit int) ([]domain.Embedding, error) {
	...
	for rows.Next() {
		...
		if err := rows.Scan(&e.ID, &e.ReportID, &e.ChunkIndex, &e.Content, &v, &e.CreatedAt); err != nil {
...
const querySimilarEmbeddings = `
	SELECT id, report_id, chunk_index, content, embedding, created_at
	FROM report_embeddings
	WHERE report_id = $1
...
Embedding 通常跟随报告或文件的生命周期变化。当报告重新生成时，应先删除旧向量再写入新向量；放在同一事务里可避免检索到"新旧混合"的中间状态：
